package test.views
import blockudoku.controllers.{ElementController, ElementControllerImpl, GridController, GridControllerImpl}
  ...
  "GridView" when {
    "size 9x9" should {
      "display a 9x9 grid" in {
        val newGridController = GridControllerImpl(9, 9, elementController, new FocusManager(Grid))
        val focusManager = new FocusManager(Grid)
        val gridView = ConsoleGridView(mediator, newGridController, ElementControllerImpl(RandomMock(), focusManager), focusManager)
    ...
    "size 4x4" should {
      "display a 4x4 grid" in {
        val newGridController = GridControllerImpl(4, 4, elementController, new FocusManager(Grid))
        val focusManager = new FocusManager(Grid)
        val gridView = ConsoleGridView(mediator, newGridController, ElementControllerImpl(RandomMock(), focusManager), focusManager)
        viewContent(gridView).replace("\r\n", "\n") should be(
          """x----x----x----x----x
    ...
    "size 6x6" should {
      "display a 6x6 grid" in {
        val newGridController = GridControllerImpl(6, 6, elementController, new FocusManager(Grid))
        val focusManager = new FocusManager(Grid)
        val gridView = ConsoleGridView(mediator, newGridController, ElementControllerImpl(RandomMock(), focusManager), focusManager)
        viewContent(gridView).replace("\r\n", "\n") should be(
          """x----x----x----x----x----x----x
    ...
    "an element is added" should {
      "display the element at the correct position" in {
        val newElementController = ElementControllerImpl(RandomMock(), new FocusManager(Grid))
        val newGridController = GridControllerImpl(6, 6, newElementController, new FocusManager(Grid))
        val focusManager = new FocusManager(Grid)
        val elementController = ElementControllerImpl(RandomMock(), focusManager)
        val gridView = ConsoleGridView(mediator, newGridController, newElementController, focusManager)
    ...
    "formatted" should {
      "select the correct tile" in {
        ...
        val gridView = ConsoleGridView(mediator, gridController, elementController, focusManager)
    ...
    "previewing" should {
      "display a green preview when the element can be placed" in {
        ...
        val elementController = ElementControllerImpl(RandomMock(), focusManager)
        elementController.selectElement(elementController.elements(0))
        val newGridController = GridControllerImpl(6, 6, elementController, focusManager)
        val gridView = ConsoleGridView(mediator, newGridController, elementController, focusManager)
